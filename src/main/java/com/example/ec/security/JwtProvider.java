...
public class JwtProvider {
    ...
    public String createToken(String username, List<Role> roles) {
        ...
        Date now = new Date();
        return Jwts.builder().setClaims(claims).setIssuedAt(now).setExpiration(new Date(now.getTime() + validityInMilliseconds)).signWith(SignatureAlgorithm.HS256, secretKey).compact();
    }
